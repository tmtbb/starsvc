...
TradesStar::TradesStar() {
    data_ = new Data();
}

TradesStar::TradesStar(const TradesStar& trads_star)
    : data_(trads_star.data_) {
    if (data_ != NULL) {
        data_->AddRef();
    }
}

TradesStar& TradesStar::operator =(const TradesStar& trades_star) {
    if (trades_star.data_ != NULL) {
        trades_star.data_->AddRef();
    }
    ...
    data_ = trades_star.data_;
    return (*this);
}

void TradesStar::ValueSerialization(base_logic::DictionaryValue* dict) {
    dict->GetString(L"symbol", &data_->symbol_);
    dict->GetString(L"name", &data_->name_);
    dict->GetString(L"wid", &data_->wid_);
    dict->GetString(L"start_time", &data_->start_time_);
    dict->GetString(L"end_time", &data_->end_time_);
    dict->GetInteger(L"trades_time", &data_->trades_time_);
    dict->GetInteger(L"interval_time", &data_->interval_time_);
}

void TradesStar:: set_is_trading(const bool is_trading_) {
