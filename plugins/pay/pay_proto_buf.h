...
class AliPayContent {
 public:
  ...
  void set_product_code(const std::string& product_code) {
    product_code_ = new base_logic::StringValue(product_code);
  }

  void set_subject(const std::string& subject) {
    subject_ = new base_logic::StringValue(subject);
  }

  void set_total_amount(const std::string& total_amount) {
    total_amount_ = new base_logic::StringValue(total_amount);
  }


  base_logic::DictionaryValue* get() {
    value_ = new base_logic::DictionaryValue();
    if (out_trade_no_ != NULL)
      value_->Set(L"out_trade_no", out_trade_no_);
    if (product_code_ != NULL)
      value_->Set(L"product_code", product_code_);
    if (total_amount_ != NULL)
      value_->Set(L"total_amount", total_amount_);
    if (subject_ != NULL)
      value_->Set(L"subject", subject_);
 ...
 private:
  ...
  base_logic::StringValue* product_code_;
  base_logic::StringValue* subject_;
  base_logic::StringValue* total_amount_;
...
class AliPayOrder {
 ...
 private:
  ...
  base_logic::StringValue* title_;  //应用名-商品名 eg.交易支付
  base_logic::FundamentalValue* price_;  // 订单总价  单位 分
  base_logic::FundamentalValue* pay_type_;  //2支付宝
  base_logic::StringValue* open_id_;//微信公众号支付必填
};
...
class IDentityCard{
 public:
  ...
  void set_id_number(const std::string& value) {
    id_number_ = new base_logic::StringValue(value);
  }
  ...
  std::string id_number() {
    std::string tmp;
    if (id_number_)
      id_number_->GetAsString(&tmp);
    return tmp;
  }
  std::string name() {
    std::string tmp;
    if (name_)
      name_->GetAsString(&tmp);
    return tmp;
  }
