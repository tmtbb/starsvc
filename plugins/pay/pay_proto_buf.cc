...
bool IDentityCard::set_http_packet(base_logic::DictionaryValue* value) {
  int32 err = 0;
  bool r = false;
  int64 uid;
  std::string tmp;
  ...
  r = value->GetString(L"id_number", &tmp);
  if (r)
    set_id_number(tmp);
  else
    return false;

  r = value->GetString(L"name", &tmp);
  if (r)
    set_name(tmp);
  else
    return false;
...
bool AliPayOrder::set_http_packet(base_logic::DictionaryValue* value) {
  int32 err = 0;
  bool r = false;
  int64 uid;
  std::string title;
...
bool AliPayServer::set_http_packet(base_logic::DictionaryValue* value) {
  int32 err = 0;
  bool r = false;
  std::string err_str;
  std::string xml_str;
...
//____________
  ...
  base_logic::ValueSerializer* deserializer =
      base_logic::ValueSerializer::Create(base_logic::IMPL_HTTP, &xml_str);
  base_logic::DictionaryValue* xml_value =
      (base_logic::DictionaryValue*) deserializer->Deserialize(&err, &err_str);

  LOG_DEBUG2("%d, [%s]",err, err_str.c_str());

  r = xml_value->GetString(L"key", &appid);
  LOG_DEBUG2("key[%s]",appid.c_str());
return false;
  r = xml_value->GetString(L"appid", &appid);
  if (r)
    set_appid(GetXmlValue(appid));
...
bool CanclePay::set_http_packet(base_logic::DictionaryValue* value) {
  int32 err = 0;
  bool r = false;
  int64 uid;
  std::string rid;
