...
void Market_Mysql::CallStarInfo(void* param, base_logic::Value* value) {
  ...
  if (num > 0) {
    while (rows = (*(MYSQL_ROW *) (engine->FetchRows())->proc)) {
	  ...
	  if (rows[1] != NULL){
	    info_value->SetString(L"star_phone", rows[1]);
	  }
	  if (rows[2] != NULL){
	    info_value->SetString(L"star_name", rows[2]);
	  }
	  if (rows[3] != NULL){
	    info_value->SetBigInteger(L"star_gender", atoll(rows[3]));
	  }
	  if (rows[4] != NULL){
	  ...
	  if (rows[6] != NULL){
	    info_value->SetString(L"star_accid", rows[6]);
	  }
	  if (rows[7] != NULL){
	  ...
	  if (rows[9] != NULL){
	    info_value->SetInteger(L"star_vip", atoll(rows[9]));
	  }
	  if (rows[10] != NULL){
	    info_value->SetReal(L"updown", atof(rows[10]));
	  }
