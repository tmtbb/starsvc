...
bool ProfitDetail::set_http_packet(base_logic::DictionaryValue* value) {
  bool r = false;
  int64 id = 0;
  int64 bank_id = 0;
  double price = 0.0;
  ...
  r = value->GetBigInteger(L"orderdate", &bank_id);
  if (r)
    set_orderdate(bank_id);
  else
    return false;

  r = value->GetString(L"starcode", &comment);
  if (r)
    set_starcode(comment);
  else
    return false;
...
bool StarSideHandle::set_http_packet(base_logic::DictionaryValue* value) {
  bool r = false;
  int64 id = 0;
  int64 tid = 0;
  int64 bigt_handle = 0;
...
bool StarSidePosition::set_http_packet(base_logic::DictionaryValue* value) {
  bool r = false;
  int64 id = 0;
  int64 big_start = 0;
  int32 start = 0;
...
bool StarSideRecharge::set_http_packet(base_logic::DictionaryValue* value) {
  bool r = false;
  int64 id = 0;
  int64 big_start = 0;
  int32 start = 0;
...
bool StarSideTransStatis::set_http_packet(base_logic::DictionaryValue* value) {
  bool r = false;
  int64 id = 0;
  int64 big_stardate = 0;
  int32 stardate = 0;
  int64 big_enddate = 0;
  int32 enddate = 0;
  int64 big_status = 0;
  ...
  std::string tmp;
  r = value->GetString(L"starcode", &tmp);
  if (r)
    set_starcode(tmp);
  else
    return false;

  r = value->GetBigInteger(L"stardate", &big_stardate);
  if (r)
    stardate = big_stardate;
  else
    return false;
  set_stardate(stardate);

  r = value->GetBigInteger(L"enddate", &big_enddate);
  if (r)
    enddate = big_enddate;
  else
    return false;
